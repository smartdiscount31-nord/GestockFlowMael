...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_customers_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_customers_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_customer_addresses_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_customer_addresses_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_quotes_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_quotes_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_quote_items_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_quote_items_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_invoices_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_invoices_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_invoice_items_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_invoice_items_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_credit_notes_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_credit_notes_updated_at
    BEFORE UPDATE ON credit_notes
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column()';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_credit_note_items_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_credit_note_items_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_payments_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_payments_updated_at
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_company_settings_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_company_settings_updated_at
    BEFORE UPDATE ON company_settings
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column()';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_mail_settings_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_mail_settings_updated_at
    BEFORE UPDATE ON mail_settings
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column()';
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_document_counters_updated_at') THEN
    EXECUTE 'CREATE TRIGGER update_document_counters_updated_at
...
BEGIN
  -- Set quote number trigger
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_quote_number') THEN
  ...
  -- Set credit note number trigger
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'set_credit_note_number') THEN
  ...
  END IF;
END $document_triggers$;

-- Create function to update invoice status based on payments
DO $invoice_status$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_invoice_status_on_payment') THEN
    ...
    BEGIN
      ...
      -- Update amount_paid
      UPDATE invoices SET amount_paid = total_paid WHERE id = COALESCE(NEW.invoice_id, OLD.invoice_id);
      ...
      IF invoice_status != 'cancelled' THEN
        ...
        ELSIF CURRENT_DATE > due_date THEN
          UPDATE invoices SET status = 'late' WHERE id = COALESCE(NEW.invoice_id, OLD.invoice_id);
        END IF;
      END IF;
...
BEGIN
  ...
  -- Customer addresses
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON customer_addresses;
  -- Quotes
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON quotes;
  -- Quote items
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON quote_items;
  -- Orders
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON orders;
  -- Order items
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON order_items;
  -- Invoices
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON invoices;
  -- Invoice items
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON invoice_items;
  -- Credit notes
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON credit_notes;
  -- Credit note items
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON credit_note_items;
  -- Payments
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON payments;
  -- Company settings
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON company_settings;
  -- Mail settings
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON mail_settings;
  -- Document counters
  DROP POLICY IF EXISTS "Allow all access for authenticated users" ON document_counters;
EXCEPTION
  WHEN OTHERS THEN
