/*
  ...
  2. Features
    ...
    - Stock allocation across multiple locations
    - Upper case enforcement for names
*/
...
CREATE FUNCTION update_product_total_stock()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE products
  SET total_stock = (
...
CREATE POLICY "stock_groups_all_access"
  ON stock_groups
  FOR ALL
  TO authenticated
  USING (true)
...
CREATE POLICY "stocks_all_access"
  ON stocks
  FOR ALL
  TO authenticated
  USING (true)
...
CREATE POLICY "product_stocks_all_access"
  ON product_stocks
  FOR ALL
  TO authenticated
  USING (true)
