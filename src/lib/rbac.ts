...
/**
 * Check if a role can access repair intake (prise en charge atelier)
 * Accessible à tous sauf COMPTABLE et PREPARATEUR_COMMANDE (si ce rôle existe)
 */
export function canAccessRepairIntake(role: Role): boolean {
  console.log('[RBAC] canAccessRepairIntake check:', { role, canAccess: can('accessRepairIntake', role) });
  return can('accessRepairIntake', role);
}

/**
 * Check if a role can manage repairs (create, edit, delete)
 * Only MAGASIN, ADMIN and ADMIN_FULL
 */
export function canManageRepairs(role: Role): boolean {
  console.log('[RBAC] canManageRepairs check:', { role, canManage: can('manageRepairs', role) });
  return can('manageRepairs', role);
}
