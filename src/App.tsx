...
function App() {
  ...
  useEffect(() => {
    ...
    const mapPathToPage = (pathname: string): string | null => {
      ...
      if (p === '/atelier/dashboard') return 'atelier-dashboard';
    ...
    const loadNotificationsAndMontantDu = async () => {
      try {
        ...
        try {
          ...
          console.log('[Notifications] Réponse HTTP:', {
            status: notifResponse.status,
            statusText: notifResponse.statusText,
            contentType: notifResponse.headers.get('content-type')
          });

          if (notifResponse.ok) {
            const contentType = notifResponse.headers.get('content-type');
            if (contentType && contentType.includes('application/json')) {
              ...
              const textContent = await notifResponse.text();
              console.error('[Notifications] Réponse non-JSON reçue:', textContent.substring(0, 200));
            }
          } else {
            const errorText = await notifResponse.text();
            console.error('[Notifications] Erreur HTTP:', notifResponse.status, errorText.substring(0, 200));
          }
        } catch (notifError) {
          console.error('[Notifications] Erreur lors du chargement des notifications:', notifError);
        }
        ...
        try {
          ...
          console.log('[Consignments] Réponse HTTP:', {
            status: consigResponse.status,
            statusText: consigResponse.statusText,
            contentType: consigResponse.headers.get('content-type')
          });

          if (consigResponse.ok) {
            const contentType = consigResponse.headers.get('content-type');
            if (contentType && contentType.includes('application/json')) {
                ...
                try {
                  ...
                  if (detailRes.ok) {
                    ...
                    if (detailContentType && detailContentType.includes('application/json')) {
                      ...
                      console.warn('[App] Réponse non-JSON pour détails stock', stockId, ':', detailText.substring(0, 200));
                    }
                  } else {
              ...
              const consigText = await consigResponse.text();
              console.error('[Consignments] Réponse non-JSON reçue:', consigText.substring(0, 200));
            }
          } else {
            const errorText = await consigResponse.text();
            console.error('[Consignments] Erreur HTTP:', consigResponse.status, errorText.substring(0, 200));
          }
        } catch (consigError) {
          console.error('[Consignments] Erreur lors du chargement des consignments:', consigError);
        }
      } catch (error) {
