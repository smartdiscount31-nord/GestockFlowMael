...
import { generateInvoicePDF, generateQuotePDF, generateCreditNotePDF } from '../../utils/pdfGenerator';
...
export const EmailSender: React.FC<EmailSenderProps> = ({
  ...
  const generatePDF = async () => {
    try {
      console.log('Generating PDF for document type:', documentType, 'ID:', documentId);

      let pdf;

      if (documentType === 'invoice') {
        // Charger les données de la facture depuis Supabase
        ...
        console.log('Invoice data loaded for PDF:', invoice);

        pdf = await generateInvoicePDF(invoice, appSettings, appSettings?.logo_url);
      } else if (documentType === 'quote') {
        // Charger les données du devis depuis Supabase
        const { data: quote, error } = await supabase
          .from('quotes')
          .select(`
            *,
            customer:customers(*),
            items:quote_items(*, product:products(id, name, sku))
          `)
          .eq('id', documentId)
          .single();

        if (error) throw error;
        console.log('Quote data loaded for PDF:', quote);

        pdf = await generateQuotePDF(quote, appSettings, appSettings?.logo_url);
      } else if (documentType === 'credit_note') {
        // Charger les données de l'avoir depuis Supabase
        ...
        console.log('Credit note data loaded for PDF:', creditNoteData);

        pdf = await generateCreditNotePDF(creditNoteData, appSettings, appSettings?.logo_url);
      }
