...
async function buildTechLabel(ticket: RepairTicketForLabels): Promise<jsPDF> {
  ...
  y = fieldLine(doc, 'MODELE :', model || '—', x, y, contentW);
  y = fieldLine(doc, 'PANNE :', panne || '—', x, y, contentW);
  y = fieldLine(doc, 'V - P :', vp, x, y, contentW);
  y = fieldLine(doc, 'PRIX :', eur(ticket.estimate_amount), x, y, contentW);
...
async function uploadPdf(buffer: Blob | Uint8Array, path: string): Promise<string> {
  const { error } = await supabase.storage.from('labels').upload(path, buffer, {
...
export async function generateRepairLabels(ticket: RepairTicketForLabels): Promise<{ clientUrl: string; techUrl: string; }>{
  ...
  const clientPath = `repairs/${ticket.id}/label-client-${safeNumber}.pdf`;
  const techPath = `repairs/${ticket.id}/label-tech-${safeNumber}.pdf`;

  const [clientUrl, techUrl] = await Promise.all([
    uploadPdf(clientBlob, clientPath),
    uploadPdf(techBlob, techPath)
  ]);

  return { clientUrl, techUrl };
}
